...
. ✅ Configuration de WireMock pour mocker les appels aux fournisseurs d'LLM
. ⏳ Optimisation des performances des tests les plus lents (à continuer)
...
. *Implémenter les mocks WireMock* : Créer des classes utilitaires pour mocker les appels aux différents fournisseurs d'LLM
. *Optimiser les performances des tests* : Identifier et accélérer les tests les plus lents
. *Activer les tests ignorés* : Retirer les annotations @kotlin.test.Ignore pour exécuter les tests
. *Configurer la priorité de configuration* : Implémenter la règle DSL > CLI paramètre > gradle.properties > Convention over Configuration
