...
# Fonction pour mesurer le tempo
time_test_execution() {
    local test_type=$1
    ...
    if [ "$use_real_llm" = "true" ]; then
        START_TIME=$(date +%s%N)
        ../gradlew test --tests "*LlmConfigurationTest*" -Ptest.use.real.llm=true --rerun-tasks --quiet
        END_TIME=$(date +%s%N)
        echo "Tests avec vrai LLM terminés"
    else
        START_TIME=$(date +%s%N)
        ../gradlew test --tests "*LlmConfigurationTest*" --rerun-tasks --quiet
        END_TIME=$(date +%s%N)
