...
. *Propriété système mal propagée* : `plugin.project.dir` était passé via `-P` (Gradle property) au lieu de `-D` (JVM system property). Le processus Gradle TestKit ne voyait pas la propriété.

. *Exception non capturée* : Le test attend un échec de build, mais l'exception `UnexpectedBuildFailure` n'était pas capturée dans le step Cucumber.

. *Mock LLM trop valide* : Le mock retournait `"actor User"` qui est du PlantUML valide, donc la boucle de correction ne s'exécutait pas.
