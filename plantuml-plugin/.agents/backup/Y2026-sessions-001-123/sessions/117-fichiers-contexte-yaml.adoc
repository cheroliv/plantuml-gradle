...
* ☒ Fichiers de contexte créés avec structure complète
* ☒ 30 comptes Gmail répartis (27 prod + 3 tests)
* ☒ Format YAML valide
* ☒ 7 providers configurés
* ☒ Séparation production/tests
...
. **Générer les clés API** sur chaque plateforme via OAuth2 Google
. **Remplacer les `XXX...`** par les vraies clés dans les deux fichiers
. **Tester la rotation** avec `./gradlew processPlantumlPrompts`
. **Vérifier l'audit logging** dans `/var/log/plantuml/`
...
* **Fichier de session** : `.agents/sessions/117-fichiers-contexte-yaml.adoc`
* **Prompt de reprise** : `PROMPT_REPRISE.adoc` mis à jour pour Session 118
