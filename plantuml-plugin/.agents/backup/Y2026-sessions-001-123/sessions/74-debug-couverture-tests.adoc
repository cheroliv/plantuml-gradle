...
| `SESSION_74_SUMMARY.adoc` | ✅ Créé | Résumé session
|===

=== 🔧 Correctifs appliqués
. ✅ **Logger SLF4J réel** dans `DiagramProcessorPrivateMethodsTest`
. ✅ **2 paramètres** pour appel réflexion `archiveAttemptHistory(history, logger)`
. ✅ **Migration mockito-kotlin** dans `PromptOrchestratorTest`
. ✅ **`mock<T>()`** pour création de mocks typés
. ✅ **`whenever()`** pour éviter conflit avec mot-clé Kotlin `when`
...
*Type* : debug
*Taille originale* : 1432 lignes (SESSIONS_HISTORY.adoc) → 50 lignes (ce fichier)