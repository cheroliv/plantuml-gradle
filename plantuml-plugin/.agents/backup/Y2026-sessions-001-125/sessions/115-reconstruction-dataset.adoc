...
| `.agents/HISTORIQUE_RECONSTRUCTION_GIT.adoc` | ~800 | Archive | Reconstruction complète via Git (115 sessions)
| `.agents/DATASET_FINETUNING_PLAN.adoc` | ~400 | Plan | Plan de création de dataset (sources, catégories, métriques)
| `.agents/DATASET_FINETUNING_MODE_OPERATOIRE.adoc` | ~900 | Procédure | Mode opératoire détaillé (6 phases, 13 scripts Python)
| `.agents/sessions/115-reconstruction-dataset.adoc` | ~50 | Archive | Archive de la session
|===
...
│   ├── archives/                 # 5 fichiers SESSIONS_*.adoc
...
*Priorités* :
. Créer/collecter des fichiers `plantuml-context.yml` avec :
...
. **Git = Source de vérité** — Les commits tagués "Session" permettent la reconstruction rétrospective
. **Archives multiples** — Sessions individuelles + archives consolidées = couverture complète
. **Dataset structuré** — JSONL + métadonnées = fine-tuning optimal
. **6 phases pour dataset** — Extraction → Transformation → Consolidation → Validation
. **13 scripts Python** — Automation complète du pipeline de distillation
...
* ☒ `PROMPT_REPRISE.adoc` mis à jour pour Session 116
* ☒ `SESSIONS_HISTORY.adoc` mis à jour
* ☒ `.agents/INDEX.adoc` mis à jour (sessions récentes)
...
*Archive* : `.agents/sessions/115-reconstruction-dataset.adoc`
