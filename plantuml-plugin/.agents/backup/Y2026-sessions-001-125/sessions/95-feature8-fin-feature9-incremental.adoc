...
. **Reprocess modified prompts** — Détection modification + reprocess
. **Cleanup outputs when prompts are deleted** — Suppression orphelins
. **Use checksum-based change detection** — SHA-256 checksums
. **Force reprocessing with clean flag** — --rerun-tasks

== Leçons apprises

. **Env vars vs System properties** — `System.getenv()` pour env vars, `System.getProperties()` pour -D flags
. **4 niveaux de priorité** — CLI > YAML > Environment > gradle.properties
. **Byte array literal** — Kotlin requiert `.toByte()` pour valeurs > 127
