...
*Problème* : Test de permission directory échouait car Gradle créait la directory manquante
...
. **Revue code** : Vérifier modifications avec `git diff`
. **Commit** : `git commit -m "fix: Session 106 -- Correction 6 tests (5 unit + 1 func)"`
. **Validation** : `git status` propre

'''

== Références

* **Archive** : `.agents/sessions/106-correction-tests-unitaires-fonctionnels.adoc` (ce fichier)
* **Reprise** : `PROMPT_REPRISE.adoc` (session 107)
* **Procédure** : `.agents/PROCEDURES.adoc`, `SESSION_PROCEDURE.adoc`
