...
_Date_: 2026-04-09  
_Dernière mise à jour_: 2026-04-22 (Session 125 — EPIC 9 KnowledgeGraphRenderer + Parser + Task, 30 tests ajoutés)
_État actuel_: 336 tests unitaires passent (100% de succès)  
_Progression_:
...
* Session 120: +10 tests (GraphifyPromptAdapterTest.kt) + 5 tests (GenerateDiagramDocsTaskTest.kt)
* Session 121: +0 tests (Migration AsciiDoc — aucun test modifié)
* Session 125: +30 tests (KnowledgeGraphRendererTest 13 + KnowledgeGraphParserTest 10 + GenerateKnowledgeGraphDiagramTaskTest 7)
...
| ReindexPlantumlRagTask | ✅ ReindexPlantumlRagUnitTest (7) + IntegrationTest (10, StubEmbedding) | ✅ IntegrationTest (StubEmbedding) | **Bonne** | ✅ OK
...
| KnowledgeGraphRenderer | ✅ KnowledgeGraphRendererTest (13 tests) | ❌ Non nécessaire | **Excellente** | ✅ OK
| KnowledgeGraphParser | ✅ KnowledgeGraphParserTest (10 tests) | ❌ Non nécessaire | **Excellente** | ✅ OK
| GenerateKnowledgeGraphDiagramTask | ✅ GenerateKnowledgeGraphDiagramTaskTest (7 tests) | ❌ Non nécessaire | **Bonne** | ✅ OK
|===
...
_Nested: WithMockDiagramProcessor_
...
=== Contexte
Après l'ajout de tests dans `PlantumlManagerTest.kt` (Session 1), une _analyse manuelle des overlaps_ a été réalisée pour identifier les tests redondants.

_Objectif_ : Maintenir une couverture _maximale_ avec un minimum de redondance, suivant les principes TDD/Clean Code.
...
_Critères de détection_ :
* ✅ _Même code YAML_ copié-collé dans plusieurs fichiers
* ✅ _Mêmes assertions_ sur les mêmes données
* ✅ _Même scénario_ testé avec la même approche
...
_Questions posées_ :
* ❓ Ce test apporte-il une _couverture unique_ ?
* ❓ Ce test détecte-t-il des _régressions différentes_ ?
* ❓ Ce test utilise-t-il une _approche différente_ (mocks vs ProjectBuilder) ?
...
_Règle_ : Garder _au moins 1 test_ par scénario, supprimer les doublons exacts.
...
_Total tests_ : 71 → _70 tests_ (-1.4%)  
_Couverture_ : _100% préservée_ (aucune régression)
...
* ❌ _Mauvais_ : 3 tests identiques pour le même scénario
* ✅ _Bon_ : 1 test unique + 2 tests complémentaires (approches différentes)
...
_Checklist avant d'ajouter un test_ :
* [ ] Ce scénario est-il _déjà couvert_ ?
* [ ] Mon test apporte-t-il une _valeur ajoutée_ (approche différente, cas limite) ?
* [ ] Ce test détectera-t-il une _régression différente_ ?
...
_Pratiques_ :
* ✅ _Analyse périodique_ des overlaps (en fin de cycle de développement)
* ✅ _Refactoring des tests_ comme le code de production
* ✅ _Suppression proactive_ des tests devenus redondants
...
_Règle_ :
* ✅ _70%+_ tests unitaires purs (<10ms)
* ✅ _20-30%_ tests avec mocks (<100ms)
* ✅ _<10%_ tests d'intégration / fonctionnels (<1s)
...
_Fréquence recommandée_ :
* ✅ _Après chaque session_ de création de tests (3-5 fichiers)
* ✅ _Avant chaque merge_ sur la branche principale
* ✅ _En pré-release_ (nettoyage complet)
...
* _Tests totaux_: 103 tests
* _Fichiers de test_: 17 fichiers
* _Couverture LlmService_: ⚠️ Partielle (méthodes privées non testées)

=== Après session 8 (LlmServicePrivateMethodsTest.kt)
* _Tests totaux_: 111 tests (_+8_)
* _Fichiers de test_: 18 fichiers (+1)
* _Couverture LlmService_: ✅ Excellente (méthodes privées testées indirectement)
* _Nouvelles protections_ :
...
=== Reste à faire
* _Classes SANS tests unitaires directs_: 0 (toutes couvertes)
* _Méthodes privées NON testées_: ~5 méthodes (DiagramProcessor)
* _Data classes SANS tests directs_: 0 (toutes testées via ModelsDataClassTest)

_Tests unitaires à créer_: ~2 fichiers de test restants, ~13-15 tests additionnels
...
* ✅ `should load config with all sections populated` — _TOUS_ les champs YAML
...
_Champs YAML testés exhaustivement_ :
...
* ✅ `should register tasks with correct types` — _types_ des tâches (ProcessPlantumlPromptsTask, etc.)
...
* `processPrompts()` - _NON TESTÉE UNITAIREMENT_
...
* `processSinglePrompt(...)` - _NON TESTÉE UNITAIREMENT_
...
* `loadConfiguration()` - _NON TESTÉE UNITAIREMENT_
...
* `validateSyntax()` - _NON TESTÉE UNITAIREMENT_
...
* `reindexRag()` - _NON TESTÉE UNITAIREMENT_
...
* `simulateIndexing(...)` - _NON TESTÉE UNITAIREMENT_
...
_Protection contre les régressions_ :
...
=== Avant session
* _Tests totaux_: 75 tests
* _Fichiers de test_: 13 fichiers
* _Couverture models.kt_: ❌ Faible (0 test direct, uniquement via YAML)

=== Après session 4 (ModelsDataClassTest.kt)
* _Tests totaux_: 86 tests (_+11_)
* _Fichiers de test_: 14 fichiers (+1)
* _Couverture models.kt_: ✅ Excellente (11 tests)
* _Nouvelles protections_ :
...
=== Reste à faire
* _Classes SANS tests unitaires directs_: 3 (tâches Gradle)
* _Méthodes privées NON testées_: ~11 méthodes (LlmService, DiagramProcessor)

_Tests unitaires à créer_: ~4 fichiers de test restants, ~25-30 tests additionnels
...
=== Avant session
* _Tests totaux_: 66 tests
* _Fichiers de test_: 12 fichiers
* _Couverture PlantumlManager_: ❌ Faible (0 test direct)

=== Après session 1 (PlantumlManagerTest.kt)
* _Tests totaux_: 71 tests (_+5_)
* _Fichiers de test_: 12 fichiers (PlantumlManagerTest.kt enrichi)
* _Couverture PlantumlManager_: ✅ Excellente (11 tests)
* _Nouvelles protections_ :
...
* _Classes SANS tests unitaires directs_: 3 (tâches Gradle)
* _Méthodes privées NON testées_: ~11 méthodes (LlmService, DiagramProcessor)
* _Data classes SANS tests directs_: 10 data classes (mais testées indirectement via YAML)

_Tests unitaires à créer_: ~6 fichiers de test restants, ~35-40 tests additionnels
...
* [ ] _Types vérifiés_ : Pas juste `assertNotNull`, mais `assertTrue(... is Type)`
* [ ] _Champs exhaustifs_ : Tous les champs testés, pas juste quelques-uns
* [ ] _Priorités testées_ : Si config multiple, tester la priorité
* [ ] _Valeurs par défaut_ : Tester les champs manquants utilisent les defaults
* [ ] _Cas limites_ : Fichier vide, YAML invalide, fichier absent
* [ ] _Protection future_ : Tester même les méthodes vides pour protéger contre régression
...
_Objectif_ : Tester la tâche de validation syntaxe
...
_Objectif_ : Tester directement les 11 data classes (déjà testées indirectement via YAML)

=== Session 4 — ProcessPlantumlPromptsTaskTest.kt (5 tests)
_Objectif_ : Tester la tâche de traitement des prompts

=== Session 5 — ReindexPlantumlRagTaskUnitTest.kt (7 tests)
_Objectif_ : Tester la tâche RAG (version unitaire, sans PGVector)

=== Session 6 — LlmServicePrivateMethodsTest.kt (8 tests)
_Objectif_ : Tester les 7 méthodes privées + timeout

=== Session 7 — DiagramProcessorPrivateMethodsTest.kt (8 tests)
_Objectif_ : Tester les 5 méthodes privées de DiagramProcessor
