...
_Fichier_ : `PlantumlWorld.kt:427-439`
...
_Fichiers_ : `CommonSteps.kt:13` ET `RagPipelineSteps.kt:14`
...
_Résultat_ : Le container créé dans `RagPipelineSteps` n'est _JAMAIS stoppé_ car `CommonSteps.cleanup()` nettoie sa propre variable (qui reste `null`).
...
_Processus actifs_ :
* `GradleDaemon 9.4.0` (Java 23) : _1.2GB RAM_
* `KotlinCompileDaemon` : _851MB RAM_
...
_Solution_ : Augmenter à 5 minutes si erreurs de startup.
...
_Solution_ : `forkEvery=1` force un redémarrage JVM qui libère les locks.
...
* _EPIC_ : `EPIC_CONSOLIDATION_TESTS_FONCTIONNELS.adoc`
* _Session_ : 90 (Correction pgvector + Memory leaks)
* _Fichiers clés_ :
...
_Date_ : 17 avr. 2026  
_Statut_ : ✅ Corrections implémentées — En attente de validation
